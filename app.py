...
def main_safe() -> None:
    """Fluxo principal com shell mais simples e robusto para Streamlit."""
    injetar_css()
    view = st.query_params.get("view", "")
    if isinstance(view, list):
        view = view[0] if view else ""
    ...
    etapas_keys = [etapa[0] for etapa in _ETAPAS_NAV]
    idx_ativo = etapas_keys.index(etapa_ativa) if etapa_ativa in etapas_keys else 0
    nav_cols = st.columns(3, gap="small")
    for i, ((chave, num, label), col) in enumerate(zip(_ETAPAS_NAV, nav_cols)):
        is_active = chave == etapa_ativa
        is_done = i < idx_ativo
        rotulo = f"OK {label}" if is_done else label
        with col:
            if st.button(
                rotulo,
                key=f"_safe_nav_btn_{num}",
                use_container_width=True,
    ...
    if etapa_ativa == "1. Dados do Empreendimento":
        st.info(
            "Preencha os dados do lancamento para gerar o relatorio completo. "
            "A analise usa dados publicos, calcula score, verba recomendada, mix de midia e PDF executivo."
        )
        col_left, col_right = st.columns([1, 1], gap="medium")
        ...
        with col_left:
            with st.container(border=True):
                ...
                st.caption("Identificacao, localizacao e volume comercial do lancamento.")
                ...
                cidade_manual = st.text_input(
                    "Ou informe a cidade",
                    placeholder="Ex: Fortaleza - CE",
                    help="Usado como alternativa se o CEP nao for reconhecido. A sugestao automatica so roda com um nome mais completo.",
                )
                ...
                if sugestao:
                    detalhes = []
                    if sugestao.get("bairro"):
                        detalhes.append(f"Bairro: {sugestao['bairro']}.")
                    if sugestao.get("rua"):
                        detalhes.append(f"Ref: {sugestao['rua']}.")
                    detalhe_str = " ".join(detalhes)
                    st.info(
                        f"Sugestao de localizacao: {sugestao['score_sugerido']}/5. "
                        f"{sugestao['resumo']} {detalhe_str}".strip()
                    )
        ...
        with col_right:
            with st.container(border=True):
                ...
                st.caption(
                    "Avalie cada atributo de 1 (pior) a 5 (melhor). "
                    "Atributos positivos reduzem o score de dificuldade."
                )
                ...
                st.caption("Ao calcular, voce concorda com os Termos de Uso disponiveis no rodape e na sidebar.")
                calcular = st.button("Calcular Score e Gerar Relatorio", type="primary", use_container_width=True)
